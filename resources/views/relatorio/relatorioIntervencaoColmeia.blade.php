...
@section('titleHeader', 'Relatório de intervenções '. $situacao_formatada. ' nas colmeias')
@section('content')
<table>
    <thead>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            <td>{{$i->id}}</td>
            <td>{{$i->colmeia->apiario->nome}}</td>
            <td>{{$i->colmeia->nome}}</td>
            <td>{{$i->descricao}}</td>
            <td>{{$i->data_inicio}}</td>
            <td>{{$i->data_fim}}</td>
        ...
        @endforeach
        <tr>
            <td colspan="7" class="grand total">
        ...
        @else
        <tr>
            <td colspan="8" style="text-align:center;font-size:11pt;color: #5D6975" class="notice">Ainda não há
                informações</td>
        </tr>
        @endif
    </tbody>
